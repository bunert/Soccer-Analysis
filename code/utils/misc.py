...
def lift_keypoints_in_3d(cam, keypoints, pad=0):
    ...
    # Make a bounding box
    x1, y1, x2, y2 = min(keypoints[:, 0])-pad, min(keypoints[:, 1]) - \
        pad, max(keypoints[:, 0])+pad, max(keypoints[:, 1])+pad
    ...
    bbox_onground = geometric.ray_plane_intersection(
        origin, bbox_direction, np.array([0, 0, 0]), np.array([0, 1, 0]))
...
def lift_box_in_3d(cam, bbox):
    ...
    for i in range(n_boxes):
        ...
        plane3d = geometric.ray_plane_intersection(
            origin, direction, np.array([0, 0, 0]), np.array([0, 1, 0]))
